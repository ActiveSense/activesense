...
public class GeneActiveProcessorTests
{
    ...
    public void SupportedType_ShouldReturnGeneActiv()
    {
        ...
        // Act & Assert
        Assert.That(_processorInterface.SupportedType, Is.EqualTo(SensorTypes.GENEActiv));
    }
    ...
    public async Task ProcessAsync_UsesCorrectArguments()
    {
        ...
        var mockArguments = new List<ScriptArgument>
        {
        ...
        Assert.That(_processor.CapturedArguments, Does.Contain("-a TRUE"));
        Assert.That(_processor.CapturedArguments, Does.Contain("-s FALSE"));
    ...
    public async Task ProcessAsync_WithEmptyArguments_ExecutesScriptWithDefaultArguments()
    {
        ...
        Assert.That(_processor.CapturedScriptPath, Is.EqualTo("mock-rscript"));
        Assert.That(_processor.CapturedArguments, Does.Contain("/mock/path/_main.R"));
        // The default arguments would be used
        Assert.That(_processor.CapturedArguments, Does.Contain("-a TRUE"));
        Assert.That(_processor.CapturedArguments, Does.Contain("-s TRUE"));
    }
}